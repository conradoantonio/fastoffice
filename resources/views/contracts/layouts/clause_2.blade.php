...
@if($contract->office->type->name == 'Física' || $contract->office->type->name == 'Virtual')
	<ul class="b-up-alpha less-li-he justify">
		<li class="one-line-sp">“EL CLIENTE” se obliga a pagar a “EL PRESTADOR” por concepto de prestación de servicios de este contrato y validando la promoción de pronto pago la cantidad mensual de ${{round( $contract->office->price / 1.10, PHP_ROUND_HALF_UP, 2 )}} ({{$contract->monthly_payment_str}}) más IVA al valor agregado la cual será válida realizando el pago el día puntual de la fecha de contratación entre el día {{$contract->payment_range_start}} y {{$contract->payment_range_end}} de cada mes.</li>
...
@else
	<ul class="b-up-alpha less-li-he justify">
		<li class="one-line-sp">“EL CLIENTE” se obliga a pagar a “EL PRESTADOR” por concepto de prestación de servicios de este contrato y validando la promoción de pronto pago la cantidad por hora de ${{round( $contract->office->price / 1.10, PHP_ROUND_HALF_UP, 2 )}} ({{$contract->monthly_payment_str}}) la cual será válida realizando el pago el día puntual de la fecha de contratación.</li>
	</ul>
@endif